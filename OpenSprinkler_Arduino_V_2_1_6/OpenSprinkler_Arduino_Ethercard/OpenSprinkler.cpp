...
int OpenSprinkler::station_pins[24] =
{
...
byte OpenSprinkler::start_network()
{
    ...
    // Wiznet W5100
    if ( !ether.begin ( ETHER_BUFFER_SIZE, ( uint8_t* ) tmp_buffer, PIN_W5100_ETHER_CS ) )
#else
    // Ethercard ENC28J60
    if ( !ether.begin ( ETHER_BUFFER_SIZE, ( uint8_t* ) tmp_buffer, PIN_ETHERCARD_CS ) )
...
void OpenSprinkler::begin()
{
    ...
    for ( int i = 0; i < SPI_DEVICES; i++ )
    {
        ...
        digitalWrite ( spi_cs_pin[i], HIGH );
    }
#endif // OPENSPRINKLER_ARDUINO
...
#ifdef OPENSPRINKLER_ARDUINO_W5100
    // Wiznet W5100
    pinMode ( PIN_W5100_SD_CS, OUTPUT );
    digitalWrite ( PIN_W5100_SD_CS, HIGH );

    if ( sd.begin ( PIN_W5100_SD_CS, SPI_HALF_SPEED ) )
        status.has_sd = 1;
#else
    // Ethercard ENC28J60
    pinMode ( PIN_ETHERCARD_SD_CS, OUTPUT );
    digitalWrite ( PIN_ETHERCARD_SD_CS, HIGH );

    if ( sd.begin ( PIN_ETHERCARD_SD_CS, SPI_HALF_SPEED ) )
        status.has_sd = 1;
#endif // OPENSPRINKLER_ARDUINO_W5100
